...
const Dashboard = () => {
  ...
  useEffect(() => {
    // Load classes
    const unsubClasses = classesService.subscribeToClasses(setClasses);
    
    // Load User Profile for Sync Status
    const unsubProfile = userService.subscribeToProfile(setProfile);

    // Load streak 
    // ... existing streak loading ...
  ...
  // Format sync time
  const getSyncStatus = () => {
    if (!profile?.lastStudySession) return 'Sync Ready';
    
    const diff = Date.now() - profile.lastStudySession.toMillis();
            ...
            <div className="flex flex-wrap justify-between items-center gap-4">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-3">
                  ...
                  <div className={`px-2 py-0.5 rounded-full text-[10px] uppercase font-bold tracking-wider flex items-center gap-1.5 border ${
                    isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-400' : 'bg-gray-50 border-gray-200 text-gray-500'
                  }`}>
                    <div className={`w-1.5 h-1.5 rounded-full ${profile?.lastStudySession ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`}></div>
                    {getSyncStatus()}
                  </div>
                </div>
