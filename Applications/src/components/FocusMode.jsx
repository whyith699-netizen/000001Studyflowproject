...
const FocusMode = () => {
  ...
  useEffect(() => {
    ...
    const unsubSessions = studySessionsService.subscribeToSessions((fetchedSessions) => {
      const pomodoroOnly = fetchedSessions.filter((session) => session.type === 'pomodoro');
              ...
              <div className={`rounded-xl p-3 border shadow-sm relative overflow-hidden ${isDarkMode ? 'sf-dark-card sf-dark-border' : 'bg-white border-gray-100'}`}>
                ...
                <div className="relative z-10">
                  <div className={`flex items-center gap-2 mb-2 pb-2 border-b ${isDarkMode ? 'border-slate-700' : 'border-gray-100'}`}>
                    ...
                    <h3 className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t('studyStreak')}</h3>
                  </div>
                  ...
                  {loading ? (
                    ...
                    sessions.map((session, index) => (
                      <div key={session.id} className="relative">
                        <div className={`absolute -left-[18px] top-1 w-2.5 h-2.5 rounded-full border-2 ${isDarkMode ? 'sf-dark-border' : 'border-white'} ${
