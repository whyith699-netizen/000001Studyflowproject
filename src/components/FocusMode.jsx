...
import { userService, examsService, studySessionsService } from '../services/firestore-service';
        ...
        <div className="flex-1 max-w-[1200px] w-full mx-auto px-6 py-8 md:px-10 md:py-10">
          ...
          <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-gray-900 text-2xl font-bold">Focus Mode</h1>
                <p className="text-gray-500 text-sm">Block out distractions with the Pomodoro technique</p>
              </div>
            </div>
          </div>
              ...
              <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-2 mb-3 pb-3 border-b border-gray-100">
                    ...
                    <h3 className="text-sm font-semibold text-gray-900">Daily Streak</h3>
                  </div>
                  <div className="flex items-end gap-2 mb-2">
                    <p className="text-4xl font-black text-gray-800">{streak}</p>
                    <p className="text-gray-500 text-sm mb-1">Days</p>
                  </div>
                  ...
                  <div className="flex gap-1 mt-4">
                    {[...Array(7)].map((_, i) => (
                      <div 
                        key={i} 
                        className={`h-1.5 flex-1 rounded-full ${i < Math.min(streak, 7) ? 'bg-orange-500' : 'bg-gray-200'}`}
                      />
                    ))}
                  </div>
                </div>
              ...
              <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
                ...
                <div className="flex flex-col gap-2">
                  {loading ? (
                    ...
                    exams.map((exam) => (
                      <div key={exam.id} className="flex items-center gap-3 p-2.5 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                        <div className="flex flex-col items-center w-10 h-10 bg-white rounded-lg border border-gray-100 text-center justify-center">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-gray-800 font-medium text-sm truncate">{exam.title}</p>
                          <p className="text-gray-400 text-[10px]">{exam.subject || 'No subject'}</p>
                        </div>
                      </div>
                    ))
                  )}
              ...
              <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm flex-1">
                ...
                <div className="relative pl-4 border-l-2 border-gray-100 space-y-4">
                  {loading ? (
                    ...
                    <div className="text-center py-6 text-gray-400 -ml-4">
                    ...
                    sessions.map((session, index) => (
                      <div key={session.id} className="relative">
                        ...
                        <p className="text-gray-400 text-xs">{session.duration}m focus</p>
                      </div>
                    ))
                  )}
                </div>
